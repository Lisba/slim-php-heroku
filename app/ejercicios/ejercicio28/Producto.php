...
class Producto
{
  ...
  public static function ListarUsuariosBD(Array $productos = NULL)
  {
    $returnValue = "";
    if($productos !== NULL)
    {
      $returnValue = "<ul>";
      $returnValue = "<li>id, codigo_de_barra, nombre, tipo, stock, precio, fecha_de_creacion, fecha_de_modificacion</li>";
      foreach($productos as $producto)
      {
        $returnValue .= "<li>{$producto[0]}, {$producto[1]}, {$producto[2]}, {$producto[3]}, {$producto[4]}, {$producto[5]}, {$producto[6]}, {$producto[7]}</li>";
      }
      $returnValue .= "</ul>";
    }
    return $returnValue;
